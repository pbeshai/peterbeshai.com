(function() {
  ...
  if (animate) {
    ...
    marksClip.attr('r', plotAreaHeight / 2 + 5);
  }
  ...
  var copySlices = d3.range(numSlices - 1).map(function(d, i) {
    return {
      id: i + 1,
      href: '#ref-slice',
      transform:
        'rotate(' +
        (i + 1) * sliceAngle * (180 / Math.PI) +
        ' ' +
        plotAreaWidth / 2 +
        ' ' +
        sliceHeight +
  ...
  points
    ...
    .style('fill', function(d) {
      return d.filled ? markColor : 'none';
    })
    .style('stroke', function(d) {
      return d.filled ? 'none' : markColor;
    });
  ...
  // add diagonal down
  var diagDown = slice
    .selectAll('.diagonalDown')
    .data(dataByType.diagonalDown || []);
  ...
  arrowMarkGs
    .append('path')
    .attr('d', function(d) {
      return toPath(dToLine(d, 'y1', 'yMid'));
    })
    .style('stroke', markColor)
    .style('fill', markColor);
  ...
  arrowMarkGs
    .append('path')
    .attr('d', function(d) {
      return toPath(dToLine(d, 'y2', 'yMid'));
    })
    .style('stroke', markColor)
    .style('fill', markColor);
})();
