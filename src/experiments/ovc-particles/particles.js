...
window.Particles = (function(d3) {
  ...
  function createParticle() {
    ...
    return {
      ...
      r: animateCircle
        ? Math.random() * options.circle.maxRadius + options.circle.minRadius
        : options.circle.minRadius,
      vx: 0,
      vy: 0,
  ...
  function updateParticles() {
    ...
    for (var i = 0; i < numParticles; i++) {
      ...
      p.vx += particleSpeed * (Math.random() - 0.5 - p.vx / 5);
      p.vy += particleSpeed * (Math.random() - 0.5 - p.vy / 5);
