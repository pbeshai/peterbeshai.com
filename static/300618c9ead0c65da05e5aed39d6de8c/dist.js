(function() {
  ...
  function highlightBrushed(brushedNodes) {
    ...
    var lis = brushOutput.selectAll('li').data(brushedNodes, function(d) {
      return d.id;
    });
    ...
    var circles = brushedCircles
      .selectAll('circle')
      .data(brushedNodes, function(d) {
        return d.id;
      });
  ...
  function toggleQuadtreeDebug() {
    ...
    if (g.select('.quadtree').size()) {
      ...
      var gQuadtree = g.insert('g', '.circles').attr('class', 'quadtree');
  ...
  d3.select('#reveal-quadtree').on('click', function() {
    return toggleQuadtreeDebug();
  });
    ...
    if (!d) {
      ...
      highlightOutput.html(
        JSON.stringify(d)
          ...
          .replace(
            /"(.+?)":/g,
            '<strong style="width: 40px; display: inline-block">$1:</strong> '
          )
          .replace(/,/g, '<br>')
      );
    }
