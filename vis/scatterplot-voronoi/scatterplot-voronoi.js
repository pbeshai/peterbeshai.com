...
function highlight(d) {
  // no point to highlight - hide the circles and clear the text
  if (!d) {
    d3.select('.highlight-circle').style('display', 'none');
    highlightOutput.text('');
  } else {
    // show the circlse at the right position
...
// otherwise we could set pointer-events: none on the other elements
g.append('rect')
  .attr('class', 'overlay')
